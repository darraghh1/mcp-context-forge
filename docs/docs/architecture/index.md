...
- Native A2A 1.0.0 passthrough at `/a2a/{name}` and `/servers/{id}/a2a/{name}` with method-aware RBAC, version negotiation, streaming dispatch, and a three-target compliance harness — see [A2A Native Passthrough](a2a-native.md)
